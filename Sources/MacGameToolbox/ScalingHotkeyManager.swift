...
public final class ScalingHotkeyManager: @unchecked Sendable {
    public static let shared = ScalingHotkeyManager()
    ...
    public func register(keyCode: UInt32, modifiers: UInt32, handler: @escaping () -> Void) {
        ...
        let id = nextID
        nextID += 1
        let hotKeyID = EventHotKeyID(signature: OSType(0x4D47_5442), id: id) // MGTB
    ...
    private func installHandler() {
        ...
        InstallEventHandler(GetApplicationEventTarget(), { (_, eventRef, userData) -> OSStatus in
            ...
            let manager = Unmanaged<ScalingHotkeyManager>.fromOpaque(userData).takeUnretainedValue()
